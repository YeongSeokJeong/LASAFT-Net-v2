...
def get_conditional_loss(loss_name, window_length, hop_length, **kwargs):
    ...
    elif loss_name == 'spec_l1_multi_source':
        return MultiSourceSpectrogramLoss('l1')
    elif loss_name == 'spec_l2_multi_source':
        return MultiSourceSpectrogramLoss('l2')
    elif loss_name == 'spec_mse_multi_source':
        return MultiSourceSpectrogramLoss('mse')
...
class MultiSourceSpectrogramLoss(MultiSourceLoss):
    ...
    def compute(self, model, mixture_signal, conditions, target_signals):
        mixture_spec = model.to_spec(mixture_signal)
        source_batch_spec = [model.to_spec(target) for target in target_signals]
        target_hats = model.forward_multi_source(mixture_signal, conditions)
        e_recon = self.criterion(target_hats, torch.stack(source_batch_spec))
